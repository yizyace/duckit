...
test('budgets across months, preserves stale category edits, and reviews historical reports', async ({}, info) => {
  ...
  try {
    ...
    const opener = page.getByRole('button', { name: /^Budget Groceries, September 2026/ })
    await opener.click()
    ...
    await expect(opener).toHaveAccessibleName(/^Budget Groceries, September 2026:/)
    await expect(opener).toHaveAccessibleName(/321\.09/)
    ...
    const nameMismatch = await new AxeBuilder({ page })
      .setLegacyMode(true)
      .withRules(['label-content-name-mismatch'])
      .analyze()
    expect(nameMismatch.violations).toEqual([])
