...
| Packaged runtime         | Recursive native-code validation and matching host/Node/app execution required; native Apple Silicon and Intel runner results are recorded in CI |
...
At the initial 0.1.0 release, the source passed 103 unit/integration tests and 12 real
Electron tests locally and in GitHub CI. Both packaged architectures passed launch
and recovery checks with developer tools absent from the application's PATH; the
initial Intel execution used Rosetta. The current [CI and delivery workflow](ci.md)
uses matching native runners and stores source-specific reports. Package smoke now
rejects translated execution before launching Electron. Actual GitHub HTTPS transport
and reuse of existing Keychain credentials through bundled GCM passed separately
from synthetic network tests. A fresh browser credential grant, fresh-Mac Gatekeeper
behavior and auditory VoiceOver testing remain unverified. Use the matching CI run's
native smoke report when assessing an installer rather than carrying forward an
older release's hardware coverage.
